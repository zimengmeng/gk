  ...
  mutations: {
    ...
    GETCOMMONPUBLICINFO (state, data) {
      ...
      if (!localStorage.getItem('sSymbolName') || !data.market[localStorage.getItem('markTitle')][localStorage.getItem('sSymbolName')]) {
        Object.keys(data.market).length > 0 && data.market[data.maket_index] && localStorage.setItem('sSymbolName', Object.keys(data.market[data.maket_index])[0])
        localStorage.setItem('markTitle', data.maket_index)
      }
      //  设置 默认语言
    ...
    ISLOGIN (state, data) {
      ...
      if (!data) {
        ...
        // 未登录可以访问的页面
        let arr = ['index', 'cms', 'register', 'trade', 'help_center', 'forgetPassword', 'login', 'notice_info', 'otcIndex', 'otc_home', 'adsHall', 'otc_personal', 'contract', 'mining', 'agentStatic', 'dubble', 'lianghua', 'simulation']
